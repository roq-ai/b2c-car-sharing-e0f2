...
import { getFleetManagers } from 'apiSdk/fleet-managers';
...
function DashboardPage() {
  ...
  const fleetManagerStat = useSWR<PaginatedInterface<FleetManagerInterface>>(
    () => `/fleet-managers`,
    () => getFleetManagers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: fleetManagerStat?.data?.totalCount,
      isLoading: fleetManagerStat.isLoading,
